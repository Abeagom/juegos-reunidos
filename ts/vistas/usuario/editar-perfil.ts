...
`;
  ...
  logica: () => {
    const form = document.getElementById("form-editar-perfil") as HTMLFormElement | null;
    const inputNombre = document.getElementById("input-nombre") as HTMLInputElement | null;
    const inputContrasena = document.getElementById("input-contrasena") as HTMLInputElement | null;
    const mensaje = document.getElementById("mensaje-form") as HTMLDivElement | null;

    const mostrarMensaje = (texto: string, esError = true) => {
      if (!mensaje) return;
    ...
    form?.addEventListener("submit", (e) => {
      ...
      const existeNombre = usuarios.some(
        (u: any) => u.nombre === nuevoNombre && u.nombre !== usuarioActual.nombre
      );
      if (existeNombre) {
