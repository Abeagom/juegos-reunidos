...
export const VistaRegistro = {
    ...
    logica: () => {
        ...
        const nombreValidaciones = document.querySelector("#nombreValidaciones");
        const contrasenaInput = document.querySelector("#contrasenaRegistro");
        const contrasenaValidaciones = document.querySelector("#contrasenaValidaciones");
        const repetirContrasenaInput = document.querySelector("#repetirContrasenaRegistro");
        const repetirContrasenaValidaciones = document.querySelector("#repetirContrasenaValidaciones");
        const emailInput = document.querySelector("#emailRegistro");
        const fechaNacimientoInput = document.querySelector("#fechaNacimientoRegistro");
        const telefonoInput = document.querySelector("#telefonoRegistro");
        const radiosSexo = document.querySelectorAll('input[name="sexoRegistro"]');
        const dispositivoSelect = document.querySelector("#dispositivoRegistro");
        const checkboxAficiones = document.querySelectorAll('input[name="aficiones"]');
        const juegosInput = document.querySelector("#cantidadJuegos");
        const modal = document.querySelector("#modalTerminos");
        const enlaceLeer = document.querySelector("#enlaceLeer"); // El link "Leer"
        const btnAceptarModal = document.querySelector("#btnAceptarModal");
        const terminosCheckbox = document.querySelector("#terminosRegistro");
        ...
        registroForm?.addEventListener("submit", function (evento) {
            ...
            const checkboxesMarcados = document.querySelectorAll('input[name="aficiones"]:checked');
            ...
            const todosValidos = valoresSeleccionados.every(valor => aficionesPermitidas.some(permitida => permitida === valor));
            ...
            const juegosValor = juegosInput?.value || "";
        ...
        function noTieneEspacios(texto) {
        ...
        function tieneFormatoFechaCorrecto(fecha) {
            // Comprueba exactamente el formato YYYY-MM-DD
            const patron = /^\d{4}-\d{2}-\d{2}$/;
            return patron.test(fecha);
        }
        function esFechaAnteriorA1900(fechaTexto) {
        ...
        function esCantidadJuegosValida(valor) {
            // Convertimos a número
            const cantidad = Number(valor);
            // Comprobaciones de seguridad (Lógica de negocio)
            const esNumero = !isNaN(cantidad);
            const esEntero = Number.isInteger(cantidad);
            const rangoValido = cantidad >= 0 && cantidad <= 1000;
        ...
        function actualizarEstadoValidacion(idElemento, esValido) {
